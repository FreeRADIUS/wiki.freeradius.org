...
The key value itself is an xlat expansion, meaning it can be made up from multiple attributes from any list, or from SQL queries, LDAP searches or the return from any other rlm_* that provides an xlat function.

Feel free to add additional use cases if you think of new and interesting ways to use this module.

Configuration
---------------
See `here <https://github.com/alandekok/freeradius-server/blob/master/raddb/mods-available/cache>`_ for the default configuration file. The config file described the module operation adequately, so it won't be repeated here.
...
or to forcibly remove entries at he end of the session
