...
We wrap `+__attribute__+` with `CC_HINT` for legacy reasons, as it provided compatibility with compilers that did not support the `+__attribute__+` markup.  `CC_HINT` has been retained as it's shorter and easier to type.
