...
#### `UNCONST`
An `UNCONST(_type, _const_ptr)` macro is available in all compilation units. It works by first casting the pointer value to a `uintptr_t` i.e. a integer type wide enough to hold a pointer value, and then, to the type specified by `_type`.

The reason why our `UNCONST` macro has a `_type` argument, is that it's slightly safer than casting to `void *` in terms of having the compiler find type issues, and that it allows inline dereferencing of fields in the `UNCONST`'d pointer (`UNCONST(a_type_t *, foo)->bar`).
...
If a diagnostic that may not be implemented by the compiler is being used, the `DIAG_ON`, `DIAG_OFF` calls can be wrapped with `DIAG_OFF(DIAG_UNKNOWN_PRAGMAS)`, `DIAG_ON(DIAG_UNKNOWN_PRAGMAS)` calls.  An initial attempt was made to implement `DIAG_(ON|OFF)_OPTIONAL` macros, but GCC doesn't seem to support multiple `_Pragma()` calls on the same line correctly.
