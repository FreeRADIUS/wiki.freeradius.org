...
- `CC_HINT(nonnull)`, `CC_HINT(nonnull(<arg_num0>, <arg_num1>, <arg_numN>))` marks up all arguments, or specific arguments to a function as being nonnull.  The compiler will then warn if a NULL pointer value is passed explicitly in a `nonnull` argument. Clang's `ubsan` (when enabled) will abort executing if a NULL pointer value is passed to a `nonnull` argument. arg_nums are indexed from 1.
...
* `CC_HINT(warn_unused_result)` require that the return value of a function be assigned to something.  This is particularly useful for:
...
* `CC_HINT(format (printf, <arg_num_fmt>, <arg_num_variadic>))` mark up a function as accepting a fmt string and arguments.  `arg_num`s are indexes from 1. `<arg_num_fmt>` is the argument number of the format string, `<arg_num_variadic>` is the argument number variadic arguments start at (`...` in the function signature).
...
```
====
* `NEVER_RETURNS` marks up a function as causing the process to exit.
...
* `UNUSED marks` up an argument as being unused (thus avoiding unused argument warnings).
...
* `NDEBUG_UNUSED` marks up an argument as being unused for non-debug builds only.
