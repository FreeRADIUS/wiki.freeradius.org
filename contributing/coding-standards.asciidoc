...
For runtime evaluation, llvm's ubsan can be used to catch NULL `nonnull` arguments.  If you're developing code, use `llvm` and pass `--enable-llvm-undefined-behaviour-sanitizer` to `./configure`, ubsan will then abort if it finds a NULL pointer which should not be NULL.
