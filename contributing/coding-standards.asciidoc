...
The function _should not_ test the arguments for NULLness.
...
For runtime evaluation, llvm's ubsan can be used to catch NULL `nonnull` arguments.  If you're developing code, use `llvm` and pass `--enable-llvm-undefined-behaviour-sanitizer`, ubsan will then abort if it finds a pointer which should not be NULL.
