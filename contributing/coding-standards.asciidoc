...
The reason why our `UNCONST` macro has a `_type` argument, is that it's slightly safer than casting to `void *` in terms of having the compiler find type issues, and that it allows inline dereferencing of fields in the `UNCONST` 'd pointer.
